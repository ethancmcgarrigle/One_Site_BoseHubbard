#include "model_Wfield_only.h"


Model_Wfield_only::Model_Wfield_only(double U, double mu, double beta, int N_terms)
  : _U(U)
  , _mu(mu)
  , _beta(beta)
  , _N(N_terms)
...
Model_Wfield_only::~Model_Wfield_only()
{
  // destructor 
...
void Model_Wfield_only::compute_E_n()
{
...
void Model_Wfield_only::compute_Z()
{
    // compute summand e{-beta * E(n)}
    std::fill(_tmp.begin(), _tmp.end(), 0.);
...
void Model_Wfield_only::compute_U()
{
  // Compute <U>
  std::fill(_tmp.begin(), _tmp.end(), 0.);
...
void Model_Wfield_only::compute_N()
{
  // Compute <U>
  std::fill(_tmp.begin(), _tmp.end(), 0.);
  std::fill(_tmp2.begin(), _tmp2.end(), 0.);
